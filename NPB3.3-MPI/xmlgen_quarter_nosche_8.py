...
link_node_router = """  <link id='linkn{1}s{0}' bandwidth='1000000000.0' latency='8.799999999999999e-07'/>
"""
...
link_router_router = """  <link id='links{0}s{1}' bandwidth='1000000000.0' latency='2.0852e-06'/>
"""
...
if __name__ == "__main__":
    ...
    edge = "quarter_nosche_8"
    
    # print(header)
    # print(link_node_router.format(0, 1, 2))

    random_numbers = random.sample(range(64),8)
    ...
    G = nx.Graph()
    G.add_nodes_from(range(0,9 + non_zero_groups))
    ...
    for name, num in group_counts.items():
      if num != 0:
        for i in range(4,4+num):
          G.add_edge(j,n)
          n += 1
        G.add_edge(j,8 + non_zero_groups)
        j += 1
      else:
        continue
    ...
    with open(os.path.join(out_dir, out_xml), "w") as f:
        ...
        for r1, r2 in edgelist:
          if (r1 >= 8 and r2 >= 8):
            if (r1 < (8 + non_zero_groups) and G.degree[r1] >= 3):
              f.write(link_router_router_upper.format(r1, r2, int((G.degree[r1] - 1) * 250000000)))
            elif (r2 < (8 + non_zero_groups) and G.degree[r2] >= 3):
              f.write(link_router_router_upper.format(r1, r2, int((G.degree[r2] - 1) * 250000000)))
            else:
              f.write(link_router_router_upper.format(r1, r2, int(250000000)))
          else:
