...
link_node_router = """  <link id='linkn{1}s{0}' bandwidth='1000000000.0' latency='8.799999999999999e-17'/>
"""
...
link_router_router = """  <link id='links{0}s{1}' bandwidth='1000000000.0' latency='2.0852e-16'/>
"""

link_router_router_upper = """  <link id='links{0}s{1}' bandwidth='{2}.0' latency='2.0852e-16'/>
"""
        ...
        for r1, r2 in edgelist:
          if (r1 >= 8 and r2 >= 8):
            if (r1 < (8 + non_zero_groups) and G.degree[r1] >= 3):
              f.write(link_router_router_upper.format(r1, r2, int((G.degree[r1] - 1) * 1000000000)))
            elif (r2 < (8 + non_zero_groups) and G.degree[r2] >= 3):
              f.write(link_router_router_upper.format(r1, r2, int((G.degree[r2] - 1) * 1000000000)))
            else:
